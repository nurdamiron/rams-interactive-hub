...
app.whenReady().then(() => {
  ...
  let currentLedMode = 'RAINBOW'; // Track current LED mode for state persistence
  ...
  ipcMain.handle('hardware-led-mode', async (_event, mode) => {
    ...
    if (upperMode === 'AUTO') {
      if (ledAutoCycleTimer) {
        stopLedAutoCycle();
        currentLedMode = 'RAINBOW'; // fallback after stopping auto
  ...
  ipcMain.handle('hardware-led-effect', async (_event, effectId, speed) => {
    ...
    currentLedMode = `EFFECT_${effectId}`; // track direct effect
  ...
  // Get current LED state (for UI persistence across modal open/close)
  ipcMain.handle('hardware-get-led-state', () => {
    return {
      mode: currentLedMode,
      autoCycle: !!ledAutoCycleTimer,
      rainbowActive: !!ledRainbowTimer,
    };
  });
